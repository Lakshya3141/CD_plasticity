...
@author: laksh
Code to run exploration simulations corresponding to Figure 6 of report
"""
...
def add_param(traj):
    ...
    traj.f_add_parameter('sp.tau', np.array([0.5, 0.5]),
...
if __name__ == '__main__':    
    ...
    # rb0 = [np.array(i) for i in rb0]

    
    fn = "Main_TauVar.hdf5"
    traje = 'dummy'
    # print(f'STARTING {i*3 + k +1} OUT OF 6')
            
    main(fn, fld, traje)
    # traj = Trajectory(traje, add_time=False)
    # traj.f_load(filename=os.path.join("hdf5","lin_sim", fn), load_parameters=2,
