...
def prim_sim(a0, b0, n0, plast, grow, A, B, Gaa, Gbb, kar, rho, tau, r, sig_s, sig_u, sig_e, sig_eps, tot, seed):
...
def plot_bas(a, b, n, sgs, sgc, epss, epsd, A, B, tot):
    plt.rcParams.update({'font.size': 8})
    figure, axis = plt.subplots(3, 2, figsize=(6,6))
    
    # trait a
    axis[0, 0].plot(range(0,tot), a[:,0], label = "a sp1", color='blue')
    axis[0, 0].plot(range(0,tot), a[:,1], label = "a sp2", color='red')
    # axis[0, 0].set_ylim(np.min(a)*(1-0.1*np.sign(np.max(a))) , np.max(a)*(1+0.1*np.sign(np.min(a))))
    axis[0, 0].legend()
    axis[0, 0].set_title("trait a")
      
    # trait b
    axis[0, 1].plot(range(0,tot), b[:,0], label = "b sp1", color='blue')
    axis[0, 1].plot(range(0,tot), b[:,1], label = "b sp2", color='red')
    
    axis[0, 1].legend()
    axis[0, 1].set_title("trait b")
      
    # population n
    axis[1, 0].plot(range(0,tot), n[:,0], label = "n sp1", color='blue')
    axis[1, 0].plot(range(0,tot), n[:,1], label = "n sp2", color='red')
    # axis[1, 0].set_ylim(np.min(n)*(1-0.1*np.sign(np.max(n))) , np.max(n)*(1+0.1*np.sign(np.min(n))))
    axis[1, 0].legend()
    axis[1, 0].set_title("population n")
    
    # trait and optima
    axis[1, 1].plot(range(0,tot), a[:,0] + b[:,0]*epsd, label = "z sp1", color='blue')
    axis[1, 1].plot(range(0,tot), a[:,1] + b[:,1]*epsd, label = "z sp2", color='red')
    ...
    axis[1, 1].legend()
    axis[1, 1].set_title("net trait and optima")
    
    # selection gradient species 1
    axis[2, 0].plot(range(0,tot), sgs[:,0], label = "sg sel sp1", color='blue')
    axis[2, 0].plot(range(0,tot), sgc[:,0], label = "sg com sp1", color='blue', linestyle="dashed")
    # axis[2, 0].set_ylim(np.min(np.array(sgc,sgs))*(1-0.1*np.sign(np.max(np.array(sgc,sgs)))) , np.max(np.array(sgc,sgs))*(1+0.1*np.sign(np.min(np.array(sgc,sgs)))))
    axis[2, 0].legend()
    axis[2, 0].set_title("Selection gradients sp 1")
    
    # selection gradient species 2
    axis[2, 1].plot(range(0,tot), sgs[:,1], label = "sg sel sp2", color='red')
    axis[2, 1].plot(range(0,tot), sgc[:,1], label = "sg com sp2", color='red', linestyle="dashed")
    # axis[2, 1].set_ylim(np.min(np.array(sgc,sgs))*(1-0.1*np.sign(np.max(np.array(sgc,sgs)))) , np.max(np.array(sgc,sgs))*(1+0.1*np.sign(np.min(np.array(sgc,sgs)))))
    axis[2, 1].legend()
    axis[2, 1].set_title("Selection gradients sp 2")
    
    plt.tight_layout()
    # Combine all the operations and display
    plt.show()
    
def plot_rep(a, b, n, sgs, sgc, epss, epsd, A, B, tot):
    plt.rcParams.update({'font.size': 8})
    figure, axis = plt.subplots(4, 1, figsize=(3,8))
...
def plot_tau(a, b, n, sgs, sgc, epss, epsd1, epsd2, A, B, tot, tau):
    plt.rcParams.update({'font.size': 11})
    figure, axis = plt.subplots(2, 1, figsize=(4,6))
    # plt.suptitle(f"tau: {tau}")
    lw = 0.6
    # trait a
    axis[0].plot(range(0,tot), a[:,0], label = "sp1", color='blue', linewidth=lw)
    axis[0].plot(range(0,tot), a[:,1], label = "sp2", color='red', linewidth=lw)
    # axis[0].set_ylim(np.min(a)*(1-0.1*np.sign(np.max(a))) , np.max(a)*(1+0.1*np.sign(np.min(a))))
    axis[0].legend(fontsize=12)
    # axis[0].set_title("trait a", fontsize=13)
    axis[0].set_xlabel("generations", fontsize=12.5)
    axis[0].set_ylabel("mean trait a", fontsize=12.5)
    # axis[0].set_ylim(3,7)
    # axis[0].set_title("trait a")
      
    # trait b
    axis[1].plot(range(0,tot), b[:,0], label = "sp1", color='blue', linewidth=lw)
    axis[1].plot(range(0,tot), b[:,1], label = "sp2", color='red', linewidth=lw)
    axis[1].legend(fontsize=12)
    # axis[0].set_title("trait a", fontsize=13)
    axis[1].set_xlabel("generations", fontsize=12.5)
    axis[1].set_ylabel("mean trait b", fontsize=12.5)
    axis[1].legend()
    # axis[1].set_title("trait b")
      
    # # population n
    # axis[2].plot(range(0,tot), n[:,0], label = "sp1", color='blue', linewidth=lw)
    # axis[2].plot(range(0,tot), n[:,1], label = "sp2", color='red', linewidth=lw)
    # # axis[1].set_ylim(np.min(n)*(1-0.1*np.sign(np.max(n))) , np.max(n)*(1+0.1*np.sign(np.min(n))))
    # axis[2].legend()
    # axis[2].set_title("population n")
    
    # # Delta z
    # axis[3].plot(range(0,tot), (a[:,0] + b[:,0]*epsd1) - a[:,1] + b[:,1]*epsd2, color='green', linewidth=lw)
    # #axis[1, 1].plot(range(0,tot), A[0] + B[0]*epss, label = "Th sp2", color='blue', linestyle='dashed')
    # #axis[1, 1].plot(range(0,tot), A[1] + B[1]*epss, label = "Th sp2", color='red', linestyle='dashed')
    # #axis[3].legend()
    # axis[3].set_title("z1 - z2")
...
def plot_tau_sep(a, b, n, sgs, sgc, epss, epsd1, epsd2, A, B, tot, tau, rho):
    plt.rcParams.update({'font.size': 11})
