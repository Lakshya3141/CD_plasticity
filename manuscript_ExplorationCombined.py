...
x_disp = 0.8
y_disp = 4
# For datas[0], colormap goes from white to orange
cm0 = plt.cm.get_cmap('Oranges')
# im0 = plt.scatter(datas[0].sig_u2 + x_disp, datas[0].sig_s2 + y_disp, c=datas[0].delz, s=size, cmap=cm0, alpha=1.0)
im0 = plt.scatter(datas[0].sig_u2, datas[0].sig_s2, c=datas[0].delz, s=size, cmap=cm0, alpha=1.0)

# For datas[2], colormap goes from white to green
cm2 = plt.cm.get_cmap('Greens')
# im2 = plt.scatter(datas[2].sig_u2 - x_disp, datas[2].sig_s2 - y_disp, c=datas[2].delz, s=size, cmap=cm2, alpha=1.0)
im2 = plt.scatter(datas[2].sig_u2, datas[2].sig_s2, c=datas[2].delz, s=size, cmap=cm2, alpha=1.0)

# For datas[1], colormap goes from white to blue
cm1 = plt.cm.get_cmap('Blues')
im1 = plt.scatter(datas[1].sig_u2, datas[1].sig_s2, c=datas[1].delz, s=size, cmap=cm1, alpha=1.0)
# im1 = plt.scatter(datas[1].sig_u2, datas[1].sig_s2, c=datas[1].delz, s=35, cmap=cm1, alpha=0.3, vmin=np.min(datas[1].delz), vmax=np.max(datas[1].delz))
